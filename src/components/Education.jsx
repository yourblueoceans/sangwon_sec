            ...
            {schools.map((school, idx) => (
                <div key={idx} className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-md hover:shadow-xl hover:border-primary-300 transition-all group relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-primary-600 font-bold text-sm mb-4">{school.major}</p>
        ...
        <div className="md:hidden flex flex-col gap-4">
            {modules.map((mod, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <button onClick={() => toggleAccordion(idx)} className="w-full flex items-center justify-between p-5 text-left">
                        ...
                        {openIndex === idx && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                <div className="p-5 pt-0 border-t border-slate-100 bg-slate-50/50">
                                    <p className="text-sm text-slate-600 leading-relaxed mb-4 mt-4 font-medium break-keep">{mod.desc}</p>
                                    <a href={getPath(mod.pdf)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-bold text-primary-600 bg-white px-3 py-2 rounded-lg border border-slate-200 shadow-sm">
...
const EduCard = ({ mod }) => (
    <a href={getPath(mod.pdf)} target="_blank" rel="noreferrer" className="bg-white p-8 rounded-[2rem] shadow-lg border border-slate-100 hover:-translate-y-3 transition-all group flex flex-col items-center text-center relative overflow-hidden hover:shadow-2xl hover:border-primary-200">
      ...
      <span className="mt-auto inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-bold group-hover:bg-primary-600 transition-colors relative overflow-hidden">
        ...
        <span className="relative z-10 flex items-center gap-2"><Icon icon="mdi:file-pdf-box" className="text-base" /> 커리큘럼</span>
      </span>
    </a>
);
