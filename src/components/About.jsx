...
const certifications = [
  { name: '정보보안기사', img: '/sangwon_sec/assets/cert-sec-engineer.jpg', date: '2025.11.15 (실기 응시)', issuer: 'KISA', desc: '정보보안 분야 최고 수준의 국가기술자격' },
  { name: 'HSE 3급 (해킹보안전문가)', img: '/sangwon_sec/assets/cert-hse.jpg', date: '2025.12.18', issuer: '한국해킹보안협회', desc: '해킹 공격 기법 이해 및 보안 기초 실무' },
  { name: '리눅스마스터 2급', img: '/sangwon_sec/assets/cert-linux.jpg', date: '2025.10', issuer: 'KAIT', desc: 'Linux 시스템 관리 및 명령어 활용 능력' },
  { name: '네트워크관리사 2급', img: '/sangwon_sec/assets/cert-network.jpg', date: '2025.04', issuer: 'ICQA', desc: '네트워크 인프라 운용 실무 능력' },
...
const experience = [
  ...
  {
    role: '해외 지점 운영 지원 & 위기 관리',
    company: '대한항공 (미주본부)',
    period: '2023.02 - 2023.09',
    desc: '미주 지역 항공 화물 데이터 분석 및 리포팅. 특히 태풍으로 인한 공항 마비 시, 12시간 동안 승객 안내 및 파트너사 비상 연락망을 가동하여 위기를 관리함.',
  },
  {
    role: 'Tech Sales Specialist',
    company: 'Computer Ahnex (USA)',
    period: '2022.02 - 2022.09',
    desc: 'McAfee 등 보안 솔루션의 기술적 스펙을 비즈니스 언어로 번역하여 고객에게 제안. 입사 3개월 만에 매장 최고 매출을 달성하며 커뮤니케이션 능력을 입증함.',
  },
];
  ...
  return (
    <section id="about" className="py-32 relative bg-bg-main overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        {/* History: Full Width */}
        <div className="bg-white rounded-[2.5rem] p-12 shadow-premium border border-white/50 relative hover:shadow-2xl transition-all duration-300 mb-16">
                ...
                {experience.map((exp, index) => (
                    <div key={index} className="relative group">
                        ...
                        <p className="text-primary-700 font-bold text-xl mb-4 flex items-center gap-2">
                            <Icon icon="mdi:account-hard-hat" className="text-2xl" /> {exp.role}
                        </p>
                        <p className="text-slate-700 text-lg leading-relaxed font-medium break-keep bg-slate-50/50 p-6 rounded-2xl border border-slate-100/50 hover:bg-slate-50 transition-colors">
                            {exp.desc}
                        </p>
                    </div>
                ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
            <div className="bg-white rounded-[2.5rem] p-10 shadow-premium border border-white/50 group">
                <h3 className="text-3xl font-extrabold text-slate-900 mb-10 flex items-center gap-4 font-heading leading-snug">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2.5rem] p-10 text-white text-center shadow-2xl relative overflow-hidden group flex flex-col justify-center">
                <div className="relative z-10">
                    ...
                    <h3 className="text-3xl font-extrabold mb-4 font-heading leading-snug">Global<br/>Capability</h3>
                    ...
                    <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-white/10 border border-white/20 text-base font-bold text-primary-200 backdrop-blur-sm">
                        <Icon icon="mdi:translate" className="text-xl" /> English: Business Comm.
                    </div>
                </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/70 backdrop-blur-md p-4" onClick={() => setSelectedCert(null)}>
                ...
                >
                    ...
                    <div className="w-full h-56 bg-slate-50 rounded-3xl mb-8 flex items-center justify-center p-6 border border-slate-100 relative">
                        <img src={selectedCert.img} alt={selectedCert.name} className="h-full object-contain drop-shadow-md" onError={(e) => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = 'Image Not Found'; }} />
                    </div>
                    <h3 className="text-2xl font-extrabold text-center text-slate-900 mb-3 font-heading break-keep">{selectedCert.name}</h3>
                    <p className="text-slate-600 text-center leading-relaxed text-base break-keep bg-slate-50 p-4 rounded-2xl mb-6 font-medium">{selectedCert.desc}</p>
